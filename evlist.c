			     ...
			     bool pipe_output, void (*exec_error)(int signo, siginfo_t *info, void *ucontext))
{
	...
	if (!evlist->workload.pid) { // eseguito dal figlio
		int ret;
		int core, socket;
		char path[256];
		FILE* f = fopen(path, "r");
		...
		if (fscanf(f, "%d", &socket)!=1) socket=-1;
		fclose(f);
		printf("the process is running on core %d, of socket %d\n",core,socket);
		fflush(stdout);


		execvp(argv[0], (char **)argv); // qui viene effettivamente lanciato il processo che poi verrà monitorato da perf
		...
		if (exec_error) {
			...
			perror(argv[0]);
		exit(-1);
	} // fine gestione figlio

	// in teoria da qui in poi solamente il padre vedrà il codice dato che il figlio ha lanciato una exec, anche 
	// se la exec dovesse fallire il figlio lancerebbe una exit per sicurezza
	if (exec_error) {
	...
	fcntl(go_pipe[1], F_SETFD, FD_CLOEXEC); // serve ad evitare che la exec lanciata dal figlio erediti i fd del padre (es la pipe)
	evlist->workload.cork_fd = go_pipe[1]; // salva qui dentro il fd della pipe in scrittura, il canale su cui scrivere per far partire il figlio
	close(child_ready_pipe[0]);
	return 0;
